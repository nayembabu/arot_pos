...
class Reports_model extends CI_Model {
	...
	public function show_sales_report(){ 
		...
		$this->db->select("a.id,a.sales_date,b.customer_name,b.customer_code,a.grand_total,a.paid_amount,a.total_sell_price_s,a.sell_payment_due");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a title='View Invoice' href='".base_url("sales/invoice/$res1->id")."'>/a></td>";
...
//		$this->db->group_by("c.`item_id`");
	...
	public function get_purchase_for_this_years($sup_id, $year)
	{
		$subquery = "
			SELECT
				trans_uniq_primary_id,
				SUM(total_sales_price_cost_sss) AS total_sales_amount,
				SUM(ttl_sale_kgs_this_product) AS total_sales_kgs,
				SUM(sales_qnty_bostas) AS total_sales_bosta
			FROM db_salesitems
			GROUP BY trans_uniq_primary_id
		";

		$this->db->select("
			COALESCE(SUM(pti.ttl_items_bosta_this_trans), 0) AS total_purchase_bosta,
			COALESCE(SUM(pti.ttl_item_kg_trans), 0) AS total_purchase_kgs,
			COALESCE(SUM(pti.ttl_trans_other_cost), 0) AS total_transport_vara,
			COALESCE(SUM(pti.ttl_com_amnt_for_trans), 0) AS total_transport_commission,
			COALESCE(SUM(pti.ghar_kuli_cost_amnt_for_trans_with_cut), 0) AS total_ghar_kuli,
			COALESCE(SUM(pti.driver_advance_amnt_cost), 0) AS total_driver_advance,
			COALESCE(SUM(pti.supp_commis_items_wsss), 0) AS total_supp_commission,
			COALESCE(SUM(pti.unpaid_amount_this_trans_tk), 0) AS total_purchase_amount,
			COALESCE(SUM(pti.koifiyat_amount_tk_for_this_trans), 0) AS total_koifiyat,

			/* এখানে sales aggregate গুলো আবার SUM করা হলো */
			COALESCE(SUM(sales_agg.total_sales_amount), 0) AS total_sales_amount,
			COALESCE(SUM(sales_agg.total_sales_kgs), 0) AS total_sales_kgs,
			COALESCE(SUM(sales_agg.total_sales_bosta), 0) AS total_sales_bosta
		");

		$this->db->from("db_purchase_transports_info pti");

		$this->db->join("($subquery) AS sales_agg",
			"pti.db_purchase_transports_info_a_idd = sales_agg.trans_uniq_primary_id",
			"left",
			FALSE
		);

		$this->db->where("pti.sup_id_ass_iddd", $sup_id);
		$this->db->where("pti.pur_date_timsssss >=", "$year-01-01");
		$this->db->where("pti.pur_date_timsssss <=", "$year-12-31");

		$query = $this->db->get();
		return $query->row();
	}

	public function get_purchase_for_this_year_month_wise($sup_id, $year)
	{
		$subquery = "
			SELECT
				trans_uniq_primary_id,
		...
		$this->db->select("
			MONTH(pti.pur_date_timsssss) AS month_num,
		...
		$this->db->group_by("MONTH(pti.pur_date_timsssss)");
		$this->db->order_by("MONTH(pti.pur_date_timsssss)", "ASC");
