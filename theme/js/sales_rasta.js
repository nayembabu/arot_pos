...
$(document).on('click', '.trans_searching_btn', function () {
    ...
    $.ajax({
      type: "post",
      url: 'sales/get_purchase_transport_info_by_date',
      data: {
...
$(document).on('change', '.selected_products_item', function () {
    ...
    $(document).on('click', '.pur_item_box', function () {
      $.ajax({
        ...
        success: function (rs) {
          ...
          let trans_cost_figures = ttl_trans_other_cost + driver_advance_amnt_cost + others_cost_amnt_for_trans;
  
          let trans_cost_per_item_wise = trans_cost_figures / ttl_items_bosta_this_trans;
  
          let cost_per_bosta = trans_com_per_bosta + ghar_kuli_rates_per_bosta + trans_cost_per_item_wise;
  
          let ttl_cost_per_kg = (trans_cost_per_item_wise / pur_kg_per_bosta);
          ...
          $('.pur_item_info_displays').html(`<table class="table table-condensed table-bordered table-striped table-responsive " >
                          ...
                          <tr>
                              <td>স্টকে আছে</td>
                              <td><span class="" >${rs.pur_item_info.due_sells_bosta_ss}</span> ${rs.pur_item_info.unit_name}</td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                              ...
                              <td><span>${ttl_cost_per_kg.toFixed(2)}</span> টাকা</td>
                          </tr>
                          <tr>
                              <td>প্রতি কেজির মোট দাম</td>
                              <td><span class="sell_rate_per_kg " >${(price_per_unit+ttl_cost_per_kg).toFixed(4)}</span> টাকা</td>
                          </tr>
                          <tr>
                              <td>১ ${rs.pur_item_info.unit_name}তে ছিলো</td>
                              <td>${rs.pur_item_info.pur_kg_per_bosta} কেজি</td>
                          </tr>
                        </table>
                        <div class="row">
                          <div class="col-md-4 ">
                              <label for="mallll1">পন্যের পরিমান </label>
                              <input type="text" id="mallll1" class="form-control ttl_qnty_items " value="" placeholder="কতো বস্তা">
                          </div>
                          <div class="col-md-4 ">
                              <label for="pon2">কতো কেজি </label>
                              <input type="text" id="pon2" class="form-control kgs_per_bosta_qnt " value="" placeholder="এক বস্তায় কতো কেজি">
                          </div>
                          <div class="col-md-4 ">
                              <label for="rattt">পন্যের দাম</label>
                              <input type="text" id="rattt" placeholder="পন্যের রেট" class="form-control item_unit_rate_per_kg " value="">
                          </div>
                        </div>
                        <center>
                          <div class="btn btn-info mx-auto add_item_in_pos_tbl " pur_trans_attr_i_get="${rs.transport_info.db_purchase_transports_info_a_idd}" pur_item_id_attrs="${rs.pur_item_info.id}" style="margin-top: 5px;">যোগ করুন</div>
                        </center>`);
        }
      });
    });
  ...
  $(document).on('click', '.add_item_in_pos_tbl', function () {
    ...
    if (kg_per_bst != '' || kg_per_bst != 0 || qnty_s_bosta != '' || qnty_s_bosta != 0 || rate_per_kg != '' || rate_per_kg != 0) {
      if ($('.add_items_id_'+id)[0]) {
        ...
        let ttl_prices = parseFloat(qnty_s_bosta) * parseFloat(kg_per_bst) * parseFloat(rate_per_kg);
        $('.pos_item_adding_tbl').append(`<tr id="row_0" class="add_items_id_${id} items_table_row " items_unq_id_attr="${id}" items_trans_id_attr="${trans_id}" salling_rates="${sell_rate_per_kg}" style="border-bottom: 2px dotted rgb(51, 122, 183);">
                          <td id="">
                          ...
                          <td id="td_0_2">
                            <input name="item_discount_0" type="text" class="form-control no-padding min_width text-center ttl_bosta_qnty_ss " value="${qnty_s_bosta}" >
                          </td>
                          <td id="">
                            <input name="item_discount_0" title="এক বস্তাতে কতো কেজি" type="text" class="form-control no-padding min_width text-center kgs_per_bosta_qntssss " value="${kg_per_bst}" >
                          </td>
                          <td id="td_0_3" class="text-right">
                            <input id="sales_price_0" name="sales_price_0" type="text" class="form-control no-padding min_width text-center rates_per_kgsss " value="${rate_per_kg}">
                          </td>
                          <td id="td_0_11" class="">
                            <input  name="td_data_0_11" type="text" class="form-control ttl_item_pricess_thiss no-padding min_width text-center ttls_saless_prices " value="${ttl_prices}">
                          </td>
  ...
  $(document).on('keyup', '.ttl_bosta_qnty_ss, .kgs_per_bosta_qntssss, .rates_per_kgsss, .ttls_saless_prices', function () {
  
    let bosta_qnt; 
    let unit_kg_per_bosta; 
    let kgs_rates;
    
    if ($.isNumeric($(this).parents('.items_table_row').find('.ttl_bosta_qnty_ss').val())) {
      bosta_qnt = $(this).parents('.items_table_row').find('.ttl_bosta_qnty_ss').val();
    } else {
      bosta_qnt = 0;
    }
  
    if ($.isNumeric($(this).parents('.items_table_row').find('.kgs_per_bosta_qntssss').val())) {
      unit_kg_per_bosta = $(this).parents('.items_table_row').find('.kgs_per_bosta_qntssss').val();
    } else {
      unit_kg_per_bosta = 0;
    }
  
    if ($.isNumeric($(this).parents('.items_table_row').find('.rates_per_kgsss').val())) {
      kgs_rates = $(this).parents('.items_table_row').find('.rates_per_kgsss').val();
    } else {
      kgs_rates = 0;
    }
    
    $(this).parents('.items_table_row').find('.ttls_saless_prices').val(parseFloat(bosta_qnt)*parseFloat(unit_kg_per_bosta)*parseFloat(kgs_rates));
  ...
  $(document).on('click', '.check_all_item_calc', function () {
    ...
    let cus_uniqs_idssss = $('.customer_uniqs_id option:selected').val();
    if (cus_uniqs_idssss != undefined) {
          ...
          $('.sales_item_calc_assigns').html(
            `<table class="table table-bordered table-striped table-responsive " style="font-size: 17px;" >
              <tr>
                  <td>মালের দাম</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control ttl_item_priceses " style="text-align: right; font-size: 18px; font-weight: bold; " value="${sum_total}">
                  </td>
              </tr>
              <tr>
                  <td>লেবার খরচ</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control lebar_cost_plus " style="text-align: right; " value="">
                  </td>
              </tr>
              <tr>
                  <td>ঘাট/ভাড়া খরচ</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control ghat_vara_cost_plus " style="text-align: right; " value="">
                  </td>
              </tr>
              <tr>
                  <td>বিক্রয় মারফত</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control sales_item_carring_system " value="">
                  </td>
              </tr>
              <tr>
                  <td>ডিসকাউন্ট</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control discount_cost_minus " style="text-align: right; " value="">
                  </td>
              </tr>
              <tr>
                  <td>সাবেক</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control previous_due_of_customers " style="text-align: right; " value="${rs.sales_due}">
                  </td>
              </tr>
              <tr>
                  <td>মোট টাকা </td>
                  <td class="  " >
                    <input type="text" name="" class="form-control ttl_customer_amountss " style="text-align: right; font-size: 18px; font-weight: bold;  " value="${sum_total+parseFloat(rs.sales_due)}">
                  </td>
              </tr>
              <tr>
                  <td>জমা</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control paid_this_sales_amountss " style="text-align: right; " value="">
                  </td>
              </tr>
              <tr>
                  <td>বকেয়া</td>
                  <td class="  " >
                    <input type="text" name="" class="form-control due_of_customer_sales_amountss " style="text-align: right; font-size: 18px; font-weight: bold;  " value="${sum_total+parseFloat(rs.sales_due)}">
                  </td>
              </tr>
            </table>
            <center class=" " >
    ...
    if(confirm("আপনি কি বিক্রয় করতে চান? ")) {
      ...
      $.ajax({
        ...
        data: {
          ...
          'kg_per_bosta[]':   $('.kgs_per_bosta_qntssss').map(function(){ return this.value; }).get(),
        ...
        success: function (rs) {
          ...
          if (rs == 0) {
            ...
            Swal.fire({
              ...
              icon: "success",
              html: `আপনার বিক্রয় সফল হয়েছে, আপনি এখন স্লিপ প্রিন্ট করে নেন <br> <a onclick="window.open('sales/sales_receipt_view_fun?sales_id=${rs}','_blank', 'width=800,height=800,left=300,top=300')" style="font-size: 18px; " > Print </a>`,
